...
For example, we have a list of email-like values and we want to match only entries that contain the Gmail domain.
The sample below intentionally uses `[at]` instead of `@` to avoid email auto-link rewriting during deployment.
